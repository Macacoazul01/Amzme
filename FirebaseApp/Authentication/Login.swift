...
class Login: BaseViewController {
    ...
    let logo: UIImageView = {
        ...
        image.image = #imageLiteral(resourceName: "ios-marketing")
        return image
    }()
    ...
    override func setup() {
        ...
        view.addConstraintsWithFormat(format: "H:|-120-[v0]-120-|", views: logo)
        ...
        stack = stackView.topAnchor.constraint(equalTo: logo.bottomAnchor, constant: 90)
        stack?.isActive = true
    ...
    @objc fileprivate func LoginClicked() {
        ...
        Auth.auth().signIn(withEmail: email, password: password) {[unowned self] (user, err) in
            ...
            let redViewController = mainview.instantiateViewController(withIdentifier: "MainTabBarController")
            let appDelegate = UIApplication.shared.delegate as! AppDelegate
            appDelegate.window?.rootViewController = redViewController
            
            Database.database().reference().child("users/profile").child(uid!).observeSingleEvent(of: DataEventType.value, with: { (snap) in
                let data = snap.value as! [String: Any]
                //UserDefaults.standard.set(data["username"] as! String, forKey: "username")
                UserDefaults.standard.set(data["typeuser"] as! String, forKey: "type")
            })
        }
    }
}
